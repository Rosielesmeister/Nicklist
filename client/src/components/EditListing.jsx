...
export function EditListing({ show, onHide, listing, onListingUpdated }) {
  const { user } = useAuth()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/products/${listing._id}`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        ...
        if (onListingUpdated) onListingUpdated(updatedListing.product || updatedListing)
        onHide()
      } else {
      ...
      <Modal.Body style={{ maxHeight: "70vh", overflowY: "auto" }}>
        ...
        <Form onSubmit={handleSubmit}>
          <Row>
            <Col md={12}>
              <Form.Group className="mb-3">
                <Form.Label>
                  Product Name *
                  <span className="text-muted ms-2">({nameCharCount}/50)</span>
                </Form.Label>
                <Form.Control
                  type="text"
                  name="name"
          ...
          <Form.Group className="mb-3">
            <Form.Label>
              Description *
              <span className="text-muted ms-2">({descriptionCharCount}/500)</span>
            </Form.Label>
            <Form.Control
              as="textarea"
              rows={4}
